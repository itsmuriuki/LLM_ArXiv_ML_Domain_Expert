...
from llm_arxiv import domain
from llm_arxiv.application import utils
from llm_arxiv.domain.cleaned_documents import CleanedPaperDocument
from llm_arxiv.domain.dataset import DatasetType, TrainTestSplit
from llm_arxiv.domain.prompt import GenerateDatasetSamplesPrompt, Prompt
...
"""
    ...
    def generate(
        ...
        for category, category_prompts in prompts.items():
            ...
            # Reduced batch size from 24 to 8 to reduce API load and improve reliability
            batches_iter = utils.misc.batch(langchain_category_prompts, size=8)
            batches_list = list(batches_iter)

            logger.info(
                f"Processing {len(category_prompts)} prompts for category '{category}' in {len(batches_list)} batches"
            )
            ...
            for i, batch in enumerate(batches_list):
                try:
                    logger.info(
                        f"Processing batch {i+1}/{len(batches_list)} for category '{category}' ({len(batch)} prompts)"
                    )
                    ...
                    if not mock and i < len(batches_list) - 1:  # Don't delay after last batch
                        import time
                        time.sleep(2)  # 2 second delay between batches
                        
                except OutputParserException:
                    logger.exception(
                        f"Failed to parse the output JSON for batch {i+1} for category {category}"
                    )
                except Exception as e:
                    logger.error(f"Error processing batch {i+1} for category {category}: {e}")
